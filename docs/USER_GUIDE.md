...
- **My components** — your composite components. Drag to place; double-click to edit its
  ...
  scope, or across the whole design. **Delete** opens a dialog that removes whole library
  *categories* (see below). The list scrolls if it
  ...
  **Move to category** dropdown below the buttons moves the selected component to an existing
  ...
  disabled until a component is selected, and while you are editing a template.
...
- **Delete categories dialog** — opened from the library's **Delete** button: it lists every
  ...
  toggle, so you can delete the whole library or just chosen categories. The list scrolls when it
  grows past ~15 categories.
- **Toast** — transient messages (e.g. "Input already has a driver").
- In every dialog, **Escape** cancels/closes it, and the confirmation dialogs pre-focus their
  primary button so **Enter** completes the action immediately.
...
4. **Group** — select a sub-circuit and press **Group** to turn it into a named composite.
...
- A D flip-flop. On the configured **Edge** of `CLK`, `Q` takes the value of `D` (after a
  clk-to-q delay); between edges `Q` holds. 
...
- The memory holds `2^AddressWidth` words of `DataWidth` bits each (so an 8-bit address is
  ...
  - Each line is `ADDR value value …` — an address followed by one or more values, all shown in the
    selected **Value format** and zero-padded to a fixed width (hex is padded to whole 4-bit
    nibbles, binary to the full bit width). The number of values per line (1, 2, 4, or 8) is chosen
    automatically to fit the editor's width.
  - **Addresses are read-only**; you edit only the values. A cursor highlights a single digit of the
...
- Two shortcuts insert a NODE *into* an existing wire, splitting it in two:
  ...
  - **Ctrl/Cmd+drag** across a wire — an imaginary dashed line is drawn, and on release a NODE is
    inserted where the line crosses the wire.
...
Press **Export Verilog** in the toolbar to generate synthesizable Verilog from the current design
(the same JSON you get from **Save JSON**) and download it as `design.v`, so the circuit can be
taken into an FPGA toolchain or an HDL simulator.
